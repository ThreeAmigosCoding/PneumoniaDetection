...
from tensorflow.keras.applications import (
    EfficientNetB0, DenseNet121, ResNet50,
...
class TransferLearningModel:
    ...
    def __init__(self, model_name, input_shape=(224, 224, 3), num_classes=2, freeze_base=True):
    ...
    def build_model(self):
        ...
        base_model_class = self.model_config['class']
        base_model = base_model_class(
            include_top=False,
            input_shape=self.input_shape
        )

        if self.model_name == 'chexnet':
            base_model.load_weights('chexnet_weights.h5', by_name=True)

        if self.freeze_base:
            base_model.trainable = False
            print(f"\n[INFO] Base {self.model_config['name']} layers frozen (will unfreeze later for fine-tuning)")
        else:
            base_model.trainable = True
            print(f"\n[INFO] Base {self.model_config['name']} layers unfrozen (training from start)")

        inputs = keras.Input(shape=self.input_shape)

        preprocess_func = self.model_config['preprocess']
        ...
        model = models.Model(inputs, outputs, name=f'{self.model_config["name"]}_Transfer')
    ...
    def compile_model(self, learning_rate=0.0001):
        ...
        print("\n" + "=" * 70)
        print(f"{self.model_config['name'].upper()} TRANSFER LEARNING MODEL")
        print("=" * 70)
    ...
    def unfreeze_base_model(self, layers_to_unfreeze=50):
        ...
        if base_model is None:
            print(f"[ERROR] Could not find base {self.model_config['name']} model!")
        ...
        fine_tune_lr = self.model_config['fine_tune_lr']
        self.model.compile(
        ...
        print(f"[INFO] Model recompiled with learning rate: {fine_tune_lr}")

    def get_callbacks(self, checkpoint_path=None):

        if checkpoint_path is None:
            checkpoint_path = f'checkpoints/{self.model_name}_best.h5'
    ...
    def train(self, train_generator, val_generator, epochs=30,
              checkpoint_path=None, fine_tune=True, fine_tune_epochs=20):

        print("\n" + "=" * 70)
        print(f"TRAINING {self.model_config['name'].upper()}")
        print("=" * 70)
        ...
        if fine_tune:
            ...
            print(f"[DEBUG] Phase 2 metrics: {list(history2.history.keys())}")
    ...
    def _merge_histories(self, hist1, hist2):
        ...
        for key in common_keys:
            try:
                ...
                print(
                    f"  ✓ Merged '{key}': {len(hist1.history[key])} + {len(hist2.history[key])} = {len(merged.history[key])} epochs")
    ...
    def plot_training_history(self, save_path=None):
        ...
        if save_path is None:
            save_path = f'results/{self.model_name}_training_history.png'
    ...
    def evaluate(self, test_generator, save_dir='results'):
        ...
        print("\n" + "=" * 70)
        print(f"EVALUATING {self.model_config['name'].upper()}")
        print("=" * 70)
        ...
        with open(f'{save_dir}/{self.model_name}_classification_report.txt', 'w') as f:
            f.write(f"{self.model_config['name'].upper()} - CLASSIFICATION REPORT\n")
            f.write("=" * 70 + "\n\n")
            f.write(report)

        self.plot_confusion_matrix(
            y_true, y_pred, class_names,
            save_path=f'{save_dir}/{self.model_name}_confusion_matrix.png'
        )
        self.plot_roc_curve(
            y_true, y_pred_proba, class_names,
            save_path=f'{save_dir}/{self.model_name}_roc_curve.png'
        )
        self.plot_precision_recall_curve(
            y_true, y_pred_proba, class_names,
            save_path=f'{save_dir}/{self.model_name}_pr_curve.png'
        )

        metrics = self.calculate_metrics(y_true, y_pred, y_pred_proba)

        with open(f'{save_dir}/{self.model_name}_metrics.json', 'w') as f:
    ...
    def plot_confusion_matrix(self, y_true, y_pred, class_names, save_path):
        ...
        plt.title(f'Confusion Matrix - {self.model_config["name"]}',
    ...
    def plot_roc_curve(self, y_true, y_pred_proba, class_names, save_path):
        ...
        plt.title(f'ROC Curve - {self.model_config["name"]}',
                  fontsize=14, fontweight='bold')
    ...
    def plot_precision_recall_curve(self, y_true, y_pred_proba, class_names, save_path):
        ...
        plt.title(f'Precision-Recall Curve - {self.model_config["name"]}',
                  fontsize=14, fontweight='bold')
    ...
    def get_model_summary(self):
        ...
        print("\n" + "=" * 70)
        print(f"{self.model_config['name'].upper()} MODEL SUMMARY")
        print("=" * 70)
        self.model.summary()
