...
class ResNet50Transfer:
    ...
    def build_model(self):
        ...
        if self.freeze_base:
            ...
            print("\n[INFO] Base ResNet50 layers frozen (will unfreeze later for fine-tuning)")
        else:
            base_model.trainable = True
            print("\n[INFO] Base ResNet50 layers unfrozen (training from start)")
    ...
    def unfreeze_base_model(self, layers_to_unfreeze=50):
        ...
        if base_model is None:
            print("[ERROR] Could not find base ResNet50 model!")
        ...
        self.model.compile(
            optimizer=Adam(learning_rate=0.00001),  # 10x niži learning rate
            loss='categorical_crossentropy',
            metrics=['accuracy',
                    keras.metrics.Precision(name='precision'),
                    keras.metrics.Recall(name='recall'),
    ...
    def get_callbacks(self, checkpoint_path='checkpoints/resnet50_best.h5'):
    ...
    def plot_training_history(self, save_path='results/resnet50_training_history.png'):
    ...
    def evaluate(self, test_generator, save_dir='results'):
        ...
        with open(f'{save_dir}/resnet50_classification_report.txt', 'w') as f:
            f.write("RESNET50 - CLASSIFICATION REPORT\n")
            f.write("="*70 + "\n\n")
            f.write(report)
        
        self.plot_confusion_matrix(y_true, y_pred, class_names,
                                   save_path=f'{save_dir}/resnet50_confusion_matrix.png')
        self.plot_roc_curve(y_true, y_pred_proba, class_names,
                           save_path=f'{save_dir}/resnet50_roc_curve.png')
        self.plot_precision_recall_curve(y_true, y_pred_proba, class_names,
                                        save_path=f'{save_dir}/resnet50_pr_curve.png')
        
        metrics = self.calculate_metrics(y_true, y_pred, y_pred_proba)
        
        with open(f'{save_dir}/resnet50_metrics.json', 'w') as f:
    ...
    def save_model(self, filepath='models/resnet50_final.h5'):
