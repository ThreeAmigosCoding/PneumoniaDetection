...
class DenseNet121Transfer:
    ...
    def build_model(self):
        ...
        if self.freeze_base:
            ...
            print("\n[INFO] Base DenseNet121 layers frozen (will unfreeze later for fine-tuning)")
        else:
            base_model.trainable = True
            print("\n[INFO] Base DenseNet121 layers unfrozen (training from start)")
    ...
    def unfreeze_base_model(self, layers_to_unfreeze=50):
        ...
        for layer in self.model.layers:
            if layer.__class__.__name__ == 'Functional' or isinstance(layer, keras.Model):
                if hasattr(layer, 'layers') and len(layer.layers) > 50:
                    base_model = layer
                    print(f"[DEBUG] Found model layer: {layer.name}, type: {type(layer)}")
                    break
        
        if base_model is None:
            try:
                for i, layer in enumerate(self.model.layers):
                    print(f"[DEBUG] Layer {i}: {layer.name} - {type(layer).__name__}")
                    if 'densenet' in layer.name.lower() or (hasattr(layer, 'layers') and len(getattr(layer, 'layers', [])) > 50):
                        base_model = layer
                        print(f"[DEBUG] Selected layer {i} as base model")
                        break
            except Exception as e:
                print(f"[ERROR] Failed to find base model: {e}")
                return
        
        if base_model is None:
            print("[ERROR] Could not find base DenseNet121 model!")
        ...
        print(f"[INFO] Model recompiled with learning rate: 0.00001")
        
    def get_callbacks(self, checkpoint_path='checkpoints/densenet121_best.h5'):
    ...
    def train(self, train_generator, val_generator, epochs=30, 
          checkpoint_path='checkpoints/densenet121_best.h5',
    ...
    def plot_training_history(self, save_path='results/densenet121_training_history.png'):
    ...
    def evaluate(self, test_generator, save_dir='results'):
        ...
        with open(f'{save_dir}/densenet121_classification_report.txt', 'w') as f:
            f.write("DENSENET121 - CLASSIFICATION REPORT\n")
            f.write("="*70 + "\n\n")
            f.write(report)
        
        self.plot_confusion_matrix(y_true, y_pred, class_names,
                                   save_path=f'{save_dir}/densenet121_confusion_matrix.png')
        self.plot_roc_curve(y_true, y_pred_proba, class_names,
                           save_path=f'{save_dir}/densenet121_roc_curve.png')
        self.plot_precision_recall_curve(y_true, y_pred_proba, class_names,
                                        save_path=f'{save_dir}/densenet121_pr_curve.png')
        
        metrics = self.calculate_metrics(y_true, y_pred, y_pred_proba)
        
        with open(f'{save_dir}/densenet121_metrics.json', 'w') as f:
    ...
    def save_model(self, filepath='models/densenet121_final.h5'):
